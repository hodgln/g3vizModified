(function (global, factory) {
    ...
    function Lollipop(target, chartType, width) {
        ...
        // data and settings
        var snvData = [], domainData = [], genePosition = [], geneSymbol;
        ...
        var _viz, _mainViz, _domainViz,
            _lollipops, _boundaries, _popLines, _pops,
        ...
        var _getDomainStart = function (d) {
            return _xScale(d[domainDataFormat.start]);
        };

        var _getDomainEnd = function (d) {
            return _xScale(d[domainDataFormat.end]);
        };
        ...
        var _updateX = function () {
            ...
            _boundaryLines//.transition().duration(_options.transitionTime)
        ...
        var _drawDomain = function () {
            ...
            let _domainG = _domainViz.append("g").attr("class", domainOpt.className.track)
                ...
                .data(domainData).enter().append("g")
            ...
            _domainRect = _domainG.append("rect")
                ...
                .attr("height", _dH)
                .attr("width", function (d) { return _getDomainRectWidth(d); })
                .attr("fill", function (d) { return domainOpt.domain.colorScheme(d[domainDataFormat.name]); });
            ...
            _domainText = _domainG.append("text")
                ...
                .attr("fill", domainOpt.domain.label.color || "black")
                .style("font", domainOpt.domain.label.font)
                .text(function (d) { return d[domainDataFormat.name]; });
        ...
        // conditional to stop popDetail from rendering if no protein change column
        ...
        var _boundaryDetailHtml = function (d) {

            let startOrStop

            if(genePosition[0] === d) {
                startOrStop = "Start"
            } else if(genePosition[1] === d) {
                startOrStop = "Stop"
            }

            let _html = 
            `<div class="note">
            ${startOrStop} position of ${geneSymbol} gene. 
            </div>`
        ...
        var _boundaryMouseOver = function (d, target) {
            if (options.tooltip) {
                _boundaryTooltip.show(d, target);
            }
        };

        var _boundaryMouseOut = function (d) {
            if (options.tooltip) {
                _boundaryTooltip.hide(d);
            }
        };
        ...
        var _drawBoundaries = function () {


            //boundary group
            _boundaries = _mainViz.append("g")
                .attr("class", boundaryOpt.boundaryClassName.group)

            _boundaryLines = _boundaries.append("g")
                .attr("clip-path", "url(#" + lollipopOpt.defsId + ")")
                .selectAll("boundary")
                .data(genePosition).enter()
                .append("line")
                .attr("x1", function (d) { return _xScale(d); })
                .attr("x2", function (d) { return _xScale(d); })
                .attr("y1", _yScale(0) + domainOpt.margin.top)
                .attr("y2", 0)
                .attr("class", boundaryOpt.boundaryClassName.line)
                .attr("stroke", boundaryOpt.boundaryLine.stroke)
                .attr("stroke-width", boundaryOpt.boundaryLine["stroke-width"]);

            _boundaryLines.on("mouseover", function (d) { _boundaryMouseOver(d); })
                .on("mouseout", function (d) { _boundaryMouseOut(d); });
        };
        ...
        var _calcAxis = function () {
            ...
            // var snvPositions = snvData.map(element => element.position)

            // minPos = Math.min(...snvPositions)

            // maxPos = Math.max(...snvPositions)

            var minPos = genePosition[0] - 1000

            var maxPos = genePosition[1] + 1000

            // console.log(genePosition[0], genePosition[1])

            // console.log(minPos, maxPos)

            // next step: change this _xRange variable and other variables to show the start position of the gene. 
            // first param the start position of the gene, (-1000 bp?), second param the largest, or end position 
            _xRange = [minPos, maxPos];
            _xScale = d3.scaleLinear().domain(_xRange).range([0, _domainW]);
            _xScaleOrig = d3.scaleLinear().domain(_xRange).range([0, _domainW]);
            _xTicks = _appendValueToDomain(_xScale.ticks(), genePosition[1], "bp");
            _xAxis = d3.axisBottom(_xScale).tickValues(_xTicks)
                .tickFormat(function (_) { return this.parentNode.nextSibling ? _ : _ + " bp"; });
        };
        ...
        var _initTooltip = function () {
            if (options.tooltip) {
                _popTooltip = tooltip().attr("class", "d3-tip").offset([8, 0])
                    ...
                    _boundaryTooltip = tooltip().attr("class", "d3-tip").offset([8, 0])
                    .direction(
                        function (d) {
                            return "n"
                        })
                    ...
                    .html(_boundaryDetailHtml);

                _mainViz.call(_popTooltip);
                _mainViz.call(_boundaryTooltip);
            }
        };
        ...
        var _initViz = function () {
            ...
            // lollipop viz - this appears to be g3_lollipop-main-(number)
        ...
        lollipop.data = {
            ...
            set genePosition(_) { genePosition = _; }, get genePosition() { return genePosition; },
            set geneSymbol(_) { geneSymbol = _; }, get geneSymbol() { return geneSymbol; },
        };
